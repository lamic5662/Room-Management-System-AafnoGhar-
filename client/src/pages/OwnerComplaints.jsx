...
export default function OwnerComplaints() {
  ...
  const load = async () => {
    try {
      ...
      showToast("error", e?.response?.data?.message || "Failed to load complaints");
    } finally {
      setLoading(false);
    }
  };
  ...
  const resolve = async () => {
    ...
    try {
      ...
      showToast("success", t("Complaint resolved ✅"));
      setOpen(false);
      await load();
    } catch (e) {
      showToast("error", e?.response?.data?.message || t("Failed to resolve"));
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <div>
      <div className="row" style={{ justifyContent: "space-between", flexWrap: "wrap" }}>
        <div>
          ...
          <p className="muted" style={{ marginTop: 6 }}>
            {t("Reply and resolve tenant issues.")}
          </p>
        </div>
      ...
      {items.length === 0 ? (
        <div className="card cardPad">{t("No complaints.")}</div>
      ) : (
            ...
            <div key={c._id} className="card cardPad">
              ...
              <div className="card cardPad" style={{ boxShadow: "none", borderRadius: 14 }}>
                ...
                <div className="muted" style={{ marginTop: 10, fontSize: 13, lineHeight: 1.6 }}>
                  <b style={{ color: "#111827" }}>{t("Complaint")}:</b> {c.title} — {c.description}
                </div>
              </div>
      ...
      <Modal
        ...
        subtitle={t("Write reply and mark as resolved.")}
